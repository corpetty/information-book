...
**Truth-value placement.** The book's framework for what happens to truth under compression. The same compressed claim can be *preserved truth* (the receiver's decoding key is close to the original), *inverted truth* (keys oppose), or *orthogonal truth* (the claim is decoded as an identity flag rather than a claim at all). Which regime occurs is set by the selection gate. See [[truth-compression-and-when-each-wins|Chapter 5c]].
